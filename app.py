...
def medicine_checker(med_input, age, health_conditions, disease_to_treat):
    ...
    for med in meds:
        ...
        if not match:
            cards.append(f"<p><b>{med}</b> not found in database.</p>")
            continue
        ...
        <div style="padding:12px; border:1px solid #ddd; border-radius:10px; margin-bottom:10px;">
            <h3>{match}</h3>
            <p><b>Dosage:</b> {MED_DB[match]['dosage']}</p>
            <p><b>Treats disease:</b> {result['treats_disease']}</p>
            <p><b>Status:</b> {suitable}</p>
            <p><b>Side Effects:</b> {', '.join(MED_DB[match]['side_effects'])}</p>
        </div>
        """
        cards.append(html)
...
def ai_chat_short(user_text, history):
    ...
    if ai_pipe:
        try:
            ...
            reply = out.replace(user_text, "").strip()
            if not reply:
                reply = "Please consult a doctor for serious symptoms."
        except Exception:
            reply = "Please consult a doctor for serious symptoms."
    else:
...
CSS = """
body { background: #f8fbff; font-family: Arial, sans-serif; }
"""
